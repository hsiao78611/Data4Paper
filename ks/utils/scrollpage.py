...
def _scroll_down(driver, count_visible_item, load_more_button_Xpath, count_scroll):
    button_location = 400 + 575 * (count_visible_item / 3)
    scroll_range = button_location

    # scroll to the position
    driver.execute_script('window.scrollTo(0,' + str(scroll_range) + ');')  # document.body.scrollHeight);') # to the bottom
    # find the button
...
def scroll_down_explore(driver, total):
    ...
    while button_visiable != 'display: none;':
        ...
        result = _scroll_down(driver, count_visible_item, load_more_button_Xpath, count_scroll)
