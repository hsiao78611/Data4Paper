...
def df_project(proj_soup, id):
    try:
        ...
        proj_founder_id = proj_soup.find(attrs = {'data-modal-title' : 'About the creator'}).get('href').split('/')[1]
            # proj_soup.find('meta', attrs={'property': 'kickstarter:creator'}).get('content').split('/')[-1]
        proj_founder_name = proj_soup.find(attrs = {'data-modal-title' : 'About the creator'}).text.strip()
        proj_location = proj_soup.find(class_ = 'mr3', href=re.compile('/discover/places/')).text.strip()
    except Exception as e:
        print 'proj_list '+str(id)+' may have a problem.'
        print e
    ...
    {'proj_id': [proj_id],
     ...
     'proj_founder_id': [proj_founder_id],
     'proj_founder_name': [proj_founder_name],
